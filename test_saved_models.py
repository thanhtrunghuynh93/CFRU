...
## try different ways to calculate distance between upperbound and delta weights
def plot_unlearn_term(log_folder_clean, log_folder_dirty, max_iters, output_file, layers=["conv2.weight", "fc2.weight"]):
    output_path = os.path.join("report", "fig", "unlearn_term")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    avg_unlearn2 = {layer: [] for layer in layers}
    avg_unlearn3 = {layer: [] for layer in layers}
    avg_delta = {layer: [] for layer in layers}
    for rid in range(max_iters):
        with open(os.path.join(log_folder_clean, "history", "history{}.pkl".format(rid)), 'rb') as test_f:
            hist_clean = CPU_Unpickler(test_f).load()
        with open(os.path.join(log_folder_dirty, "history", "history{}.pkl".format(rid)), 'rb') as test_f:
            hist_dirty = CPU_Unpickler(test_f).load()
        delta = hist_clean['server_model'] - hist_dirty['server_model']
        for layer in layers:
            weight_layer_clean = hist_clean['server_model'].state_dict()[layer].reshape(-1,)
            weight_layer_dirty = hist_dirty['server_model'].state_dict()[layer].reshape(-1,)
            cdist = nn.CosineSimilarity(dim=0, eps=1e-6)
            pdist = nn.PairwiseDistance(p=2)
            import pdb
            pdb.set_trace()
            delta_weight = delta.state_dict()[layer].cpu().detach().numpy()
            delta_weight = delta_weight.reshape(-1,)
            avg_delta[layer].append(np.mean(np.absolute(delta_weight)))
            # avg_delta[layer].append(np.linalg.norm(delta_weight))
            dirty_weight_2 = hist_dirty['unlearn_term_algo2'].state_dict()[layer].cpu().detach().numpy()
            dirty_weight_2 = dirty_weight_2.reshape(-1,)
            dirty_weight_3 = hist_dirty['unlearn_term_algo3'].state_dict()[layer].cpu().detach().numpy()
            dirty_weight_3 = dirty_weight_3.reshape(-1,)
            # avg_unlearn2[layer].append(np.linalg.norm(dirty_weight_2))
            # avg_unlearn3[layer].append(np.linalg.norm(dirty_weight_3))
            avg_unlearn2[layer].append(np.mean(np.absolute(dirty_weight_2)))
            avg_unlearn3[layer].append(np.mean(np.absolute(dirty_weight_3)))
    for layer in layers:
      plt.plot(avg_delta[layer], label= "{}-diff_clean_dirty".format(layer))
      plt.plot(avg_unlearn2[layer], label= "{}-simple-upperbound".format(layer))
      plt.plot(avg_unlearn3[layer], label= "{}-proposed-upperbound".format(layer))
      plt.legend()
      plt.xlabel('round')
      plt.ylabel('weight')
      plt.savefig(os.path.join(output_path, "{}_{}.png".format(output_file, layer)))
      plt.close()

def plot_unlearn_term_L2(log_folder_clean, log_folder_dirty, max_iters, output_file, layers=["conv2.weight", "fc2.weight"]):
    output_path = os.path.join("report", "fig", "unlearn_term")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    avg_unlearn2 = {layer: [] for layer in layers}
    ...
    for rid in range(max_iters):
        ...
        for layer in layers:
            weight_layer_clean = hist_clean['server_model'].state_dict()[layer].reshape(-1,)
            weight_layer_dirty = hist_dirty['server_model'].state_dict()[layer].reshape(-1,)
            dirty_weight_2 = hist_dirty['unlearn_term_algo2'].state_dict()[layer].reshape(-1,)
            dirty_weight_3 = hist_dirty['unlearn_term_algo3'].state_dict()[layer].reshape(-1,)
            pdist = nn.PairwiseDistance(p=2)
            delta_weight = pdist(weight_layer_clean, weight_layer_dirty)
            avg_delta[layer].append(delta_weight.item())
            avg_unlearn2[layer].append(pdist(weight_layer_clean - weight_layer_dirty, dirty_weight_2).item())
            avg_unlearn3[layer].append(pdist(weight_layer_clean - weight_layer_dirty, dirty_weight_3).item())
...
def plot_unlearn_term_L1(log_folder_clean, log_folder_dirty, max_iters, output_file, layers=["conv2.weight", "fc2.weight"]):
    output_path = os.path.join("report", "fig", "unlearn_term")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    avg_unlearn2 = {layer: [] for layer in layers}
...
def plot_unlearn_term_cdist(log_folder_clean, log_folder_dirty, max_iters, output_file, layers=["conv2.weight", "fc2.weight"]):
    output_path = os.path.join("report", "fig", "unlearn_term")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    avg_unlearn2 = {layer: [] for layer in layers}
    ...
    for rid in range(max_iters):
        ...
        for layer in layers:
            ...
            dirty_weight_3 = hist_dirty['unlearn_term_algo3'].state_dict()[layer].reshape(-1,)
            cdist = nn.CosineSimilarity(dim=0, eps=1e-6)
            delta_weight = cdist(weight_layer_clean, weight_layer_dirty)
            avg_delta[layer].append(delta_weight.item())
            avg_unlearn2[layer].append(cdist(weight_layer_clean - weight_layer_dirty, dirty_weight_2).item())
            avg_unlearn3[layer].append(cdist(weight_layer_clean - weight_layer_dirty, dirty_weight_3).item())
...
def plot_unlearn_term_edist(log_folder_clean, log_folder_dirty, max_iters, output_file, layers=["conv2.weight", "fc2.weight"]):
    def edist(first_tensor, second_tensor):
        return (first_tensor - second_tensor).pow(2).sum().sqrt()
    output_path = os.path.join("report", "fig", "unlearn_term")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    avg_unlearn2 = {layer: [] for layer in layers}
    ...
    for rid in range(max_iters):
        ...
        for layer in layers:
            ...
            dirty_weight_2 = hist_dirty['unlearn_term_algo2'].state_dict()[layer].reshape(-1,)
            dirty_weight_3 = hist_dirty['unlearn_term_algo3'].state_dict()[layer].reshape(-1,)
            delta_weight = edist(weight_layer_clean, weight_layer_dirty)
            avg_delta[layer].append(delta_weight.item())
            avg_unlearn2[layer].append(edist(weight_layer_clean - weight_layer_dirty, dirty_weight_2).item())
            avg_unlearn3[layer].append(edist(weight_layer_clean - weight_layer_dirty, dirty_weight_3).item())
...
def plot_accuracy(log_folder, dirty_name, max_iters, output_file):
    ...
    for rid in range(max_iters):
        ...
        main_acc.append(hist_dirty['accuracy'][0])
...
if __name__ == '__main__':
    ...
    # for rid in range(6):
    #     with open(os.path.join('./fedtasksave/mnist_cnum100_dist2_skew0.5_seed0/R5_P0.30_AP0.20_Alpha1.0_6atk_minus0.09', "record", "history{}.pkl".format(rid)), 'rb') as test_f:
    #         hist_clean = CPU_Unpickler(test_f).load()
    #     print(hist_clean['accuracy2'])
    ...
    # main_acc = [0.1527,0.2354,0.323,0.3994,0.4636,0.5151,0.561,0.5963,0.6262,0.6509,0.6747,0.6933,0.7102,0.7241,0.7357,0.7462,0.7565,0.7647,0.7716,0.779,0.7859,0.791,0.7959,0.8012,0.8063,0.8115,0.8149,0.8195,0.8228,0.8267,0.8305,0.8339,0.8375,0.8418,0.8445,0.8471,0.8491,0.8517,0.8548,0.8588,0.8613,0.8639,0.8658,0.8687,0.871,0.8736,0.8759,0.8776,0.8801,0.8811,0.8827,0.8845,0.8853,0.8869,0.8888,0.8903,0.8915,0.8934,0.8946,0.8961,0.897,0.8981,0.8997,0.9006,0.9017,0.9027,0.9039,0.9049,0.9055,0.9065,0.9079,0.9088,0.9098,0.9104,0.9112,0.912,0.9125,0.913,0.9137,0.9147,0.9152,0.9159,0.9165,0.917,0.9176,0.918,0.9191,0.9198,0.9202,0.921,0.9215,0.9223,0.9229,0.9232,0.9236,0.9243,0.925,0.9253,0.9258,0.926,0.9261]
    main_acc = [0.004207573695236303,0.012622721085708907,0.015427770215866441,0.02244039304126028,0.02805049140607157,0.032258065101307874,0.03506311423146541,0.03646563879654417,0.03927068792670171,0.04067321249178048,0.04207573705685925,0.04207573705685925,0.04908835988225308,0.053295933681985604,0.05750350737722191,0.05610098291663936,0.0546984583515606,0.05610098291663936,0.05189340911690684,0.05049088455182807,0.04908835988225308,0.04768583531717432,0.04628331075209555,0.04488078618701678,0.04347826151744179,0.04347826151744179,0.04628331064759932,0.04768583521267809,0.049088359777756864,0.049088359777756864,0.049088359777756864,0.049088359777756864,0.049088359777756864,0.049088359777756864,0.049088359777756864,0.04628331064759932,0.04768583521267809,0.04628331064759932,0.04628331064759932,0.04628331064759932,0.04768583521267809,0.04628331064759932,0.049088359777756864,0.049088359777756864,0.049088359777756864,0.049088359777756864,0.04768583521267809,0.04488078608252056,0.04488078608252056,0.04628331064759932,0.04628331064759932,0.04768583521267809,0.04768583521267809,0.049088359777756864,0.049088359777756864,0.05049088434283563,0.04768583521267809,0.04768583521267809,0.04768583521267809,0.049088359777756864,0.04768583521267809,0.04768583531717432,0.04908835988225308,0.04908835988225308,0.04908835988225308,0.04908835988225308,0.05049088444733185,0.05049088444733185,0.04908835988225308,0.04908835988225308,0.04768583531717432,0.04908835988225308,0.04908835988225308,0.04768583531717432,0.04768583531717432,0.04488078618701678,0.04488078618701678,0.04488078618701678,0.04628331075209555,0.04628331075209555,0.04768583531717432,0.04768583531717432,0.04768583531717432,0.04768583531717432,0.04768583531717432,0.04768583531717432,0.04768583531717432,0.04768583531717432,0.04768583531717432,0.04908835988225308,0.04768583531717432,0.04908835988225308,0.04908835988225308,0.04908835988225308,0.04908835988225308,0.04908835988225308,0.04908835988225308,0.051893409012410616,0.051893409012410616,0.051893409012410616,0.051893409012410616]
    
    plot_accuracy('./result', 'R100_P0.30_AP0.20_Alpha1.0_{}atk_minus{}'.format(atk, alp), rid, "{}atker_test_minus{}".format(atk, alp))
    # acc = []
    # for id in range(35):
