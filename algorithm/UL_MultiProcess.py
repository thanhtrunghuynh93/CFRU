...
class Client():
	...
	def process_grad(self, server_model, negative_items):
		...
		if self.num_threads <= 1:
			...
			M_v = fmodule._model_sub_multiprocessing(self.model, server_model).to('cpu')
	...
	def reply(self, svr_pkg):
		...
		# Neu khong dung copy.deepcopy thi khi truyen vao ham train, self.model cung se bi thay doi
		if not fmodule.device:
			fmodule.device = torch.device('cuda:0')
