...
class Server():
	def __init__(self, option, model, clients, test_data = None, backtask_data = None):
		...
		self.path_save = os.path.join('fedtasksave', self.option['task'],
									"ULEfficient_{}_R{}_P{:.2f}_alpha{}_seed{}_{}".format(
										option['model'],
										option['num_rounds'],
										option['proportion'],
										self.alpha,
	...
	def run(self):
		...
		eval_metric_w = self.test(w_u_i)
		print("Evaluate before a few training rounds: ", eval_metric_w)
		print("Evaluate after a few training rounds: ", eval_metric)
		self.save_models(eval_metric, unlearn_time, unlearn_GM, eval_metric_w)
		print("------------End---------------")
		return unlearn_GM
	...
	def unlearning(self, old_GMs, old_CMs):
		...
		# forget_client = self.forget_client_idx
		w_ref = self._cal_w_ref(global_model, client_models)
		list_atk_model = []
		for forget_client in self.forget_client_idxs:
			w_u = copy.deepcopy(client_models[forget_client])
			v = copy.deepcopy(client_models[forget_client])
			v_state_dict = v.state_dict()
			# model = client_models[forget_client]
			data_loader = self.clients[forget_client].calculator.get_data_loader(self.clients[forget_client].train_data, batch_size=self.clients[forget_client].batch_size)
			for iter in range(20):
				data_loader.dataset.ng_sample_original()
				for batch_id, batch_data in enumerate(data_loader):
					temp_state_dict = w_u.state_dict()
					w_u.zero_grad()
					loss = self.calculator.get_loss(w_u, batch_data, self.option)
					loss.backward()
					params = list(w_u.parameters())
					for i, (layer, value) in enumerate(temp_state_dict.items()):
						if "norm" in layer:
							continue
						temp_state_dict[layer] += params[i].grad
					w_u.load_state_dict(temp_state_dict)
					for layer in w_u.state_dict().keys():
						v_state_dict[layer] = self._l2_projection(w_u.state_dict()[layer], w_ref.state_dict()[layer])
					w_u.load_state_dict(v_state_dict)
			list_atk_model.append(w_u)
		return fmodule._model_average(list_atk_model, p=[1.0/len(self.forget_client_idxs) for _ in range(len(self.forget_client_idxs))])
	
	def _l2_projection(self, w, w_ref, radius=3):
		v = torch.div(w, max(radius, torch.norm(w, p=2)))
		return torch.add(v, w_ref)
		
	def _cal_w_ref(self, global_model, client_models):
		w_ref = copy.deepcopy(global_model)
		return_state_dict = global_model.state_dict()
		N = len(client_models)
		for layer in return_state_dict.keys():
			return_state_dict[layer] *= 0 
			return_state_dict[layer] += N*global_model.state_dict()[layer] #- client_models[self.forget_client_idx].state_dict()[layer]
			for idx in self.forget_client_idxs:
				return_state_dict[layer] -= client_models[idx].state_dict()[layer]
			return_state_dict[layer] /= (N - len(self.forget_client_idxs))
		w_ref.load_state_dict(return_state_dict)
		return w_ref
	
	def save_models(self, main_accuracy, unlearn_time, unlearn_model, list_eval_w):
		# log
		save_logs = {
			"time": unlearn_time,
			"unlearn_model": unlearn_model,
			"eval_after_FLtrain": main_accuracy,
			"eval_before_FLtrain": list_eval_w
		}
