...
def predict_fast(model, num_user, num_item, parallel_users, predict_data=None):
    model.eval()
    ...
    for s in range(int(num_user/parallel_users)):
        ...
        for i in range(s*parallel_users, (s+1)*parallel_users):
            user_input.append(torch.ones(predict_data.shape[1], 1, device=device) * i)
...
def predict_pos(model, num_user, max_posid, parallel_users, predict_data=None):
    model.eval()
    device = next(model.parameters()).device  # get the device of model
    scores = []
    for s in range(int(num_user/parallel_users)):
        ...
        item_input = []
        for i in range(s*parallel_users,(s+1)*parallel_users):
            user_input.append(torch.ones((len(predict_data[i]), 1), device=device) * i)
